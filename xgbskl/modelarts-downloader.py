...
MA_TYPE_ALGORITHM = 'algorithm'

MA_ALGORITHM_CODE_TREE_ENV = 'MA_ALGORITHM_CODE_TREE'
MA_ALGORITHM_CODE_SIZE_ENV = 'MA_ALGORITHM_CODE_SIZE'

CODE_TREE_NAME_FILED = 'name'
CODE_TREE_CHILDREN_FILED = 'children'
...
parser.add_argument('--skip-creating-dir', action='store_true', help='skip creating the dir in the recursive mode')
parser.add_argument('--type', type=str, default='common', help='set type to algorithm in the algorithm training case')
...
label = '[ModelArts Service Log]'
logging.basicConfig(level=logging.INFO,
                    format=label + '%(asctime)s - %(filename)s[line:%(lineno)d] - %(levelname)s: %(message)s')
...
def load_to_local_by_moxing(recursive, src_url, dest_directory):
    if not dest_directory.endswith('/') and not dest_directory.endswith('\\\\'):
        dest_directory = dest_directory + os.sep

    # src_url is like "s3://a/b/", dir_name is "a"
    dir_name = src_url[src_url[:-1].rfind('/') + 1:]
    if not recursive:
        if src_url.endswith('/'):
            dest_directory_path = dest_directory + dir_name
            make_dir(dest_directory_path)
            log_warn("Create a empty directory '%s' in '%s' " % (dir_name, dest_directory))
        else:
            try:
                file_name = src_url[src_url.rfind('/') + 1:]
                dest_file = dest_directory.replace('/', os.sep) + file_name
                mox.file.copy(src_url, dest_file)
                return True
            except Exception as e:
                raise DownloadException('Download single file failed: ' + str(e))
    else:
        try:
            if args.type == MA_TYPE_ALGORITHM:
                load_to_local_by_code_tree(src_url, dest_directory)
            else:
                if not args.skip_creating_dir:
                    dest_directory += dir_name
                mox.file.copy_parallel(src_url, dest_directory)
            return True
        except Exception as e:
            raise DownloadException('Download directory failed: ' + str(e))


def load_to_local_by_code_tree(src_url, dest_directory):
    code_tree_str = os.environ[MA_ALGORITHM_CODE_TREE_ENV]
    code_tree_root_node = json.loads(code_tree_str)

    # src_url endswith code_dir, remove it by the root node of code tree
    name_str = code_tree_root_node[CODE_TREE_NAME_FILED]
    src_url = src_url[:src_url.rfind(name_str)]

    handle_code_tree_node(code_tree_root_node, src_url, dest_directory, '')


def handle_code_tree_node(tree_node, s3_root_path, local_root_path, s3_path):
    # name must exist
    name_str = tree_node[CODE_TREE_NAME_FILED]
    s3_path += name_str

    if name_str.endswith('/'):
        # dir case
        local_path = local_root_path + s3_path
        os.mkdir(local_path)

        if CODE_TREE_CHILDREN_FILED not in tree_node:
            # empty dir
            return

        children_array = tree_node[CODE_TREE_CHILDREN_FILED]
        for children_tree_node in children_array:
            handle_code_tree_node(children_tree_node, s3_root_path, local_root_path, s3_path)
    else:
        # file case
        mox.file.copy(s3_root_path + s3_path, local_root_path + s3_path)


def upload_to_obs_by_moxing(trace, src_url, dst_url):
    if os.path.exists(src_url):
        if not dst_url.endswith('/'):
            dst_url = dst_url + '/'
        ...
        if os.path.isfile(src_url):
            filename = os.path.basename(src_url)
            try:
                dst_file = dst_url + filename
                # check if the single file we are trying to upload already exists in obs
                if mox.file.exists(dst_file):
                    log_warn("File '%s' is already exists at '%s'. Skip upload" % (filename, dst_url))
                else:
                    write_trace_file_if_needed(trace, src_url, STATE_UPLOADING)
                    mox.file.copy_parallel(src_url, dst_file)
                write_trace_file_if_needed(trace, src_url, STATE_UPLOADED)
                return True
            except Exception as e:
                raise e
            ...
            try:
                ...
                if mox.file.exists(dst_url + dir_name):
                    log_warn("directory '%s' is already exists at '%s'. Skip upload" % (dir_name, dst_url))
                else:
                    write_trace_file_if_needed(trace, src_url, STATE_UPLOADING)
                    mox.file.copy_parallel(src_url, dst_url + dir_name)
                write_trace_file_if_needed(trace, src_url, STATE_UPLOADED)
                return True
            except Exception as e:
                raise e
...
def get_obs_path_size(obs_path):
    if args.type == MA_TYPE_ALGORITHM:
        return int(os.environ[MA_ALGORITHM_CODE_SIZE_ENV])
    return mox.file.get_size(obs_path, recursive=True)


def judge_path_size_overflow(source_url, destination_url):
    st = os.statvfs(destination_url)
    local_path_size = st.f_bavail * st.f_frsize
    obs_path_size = get_obs_path_size(source_url)
    if local_path_size < obs_path_size:
        local_path_size /= 1024 * 1024 * 1024.
        obs_path_size /= 1024 * 1024 * 1024.
        raise DownloadException('No space left: OBS directory %s space is %.2fG, but local avail space is %.2fG' %
                                (source_url, obs_path_size, local_path_size))
...
if __name__ == '__main__':
    ...
    if args.type == MA_TYPE_ALGORITHM:
        # hide code src_url in the algorithm case
        args.src = '***'
    ...
    if is_download_to_local is not None:
        try:
            if is_download_to_local:
                judge_path_size_overflow(source_url, destination_url)
                is_success = load_to_local_by_moxing(args.recursive, source_url, destination_url)
            else:
                is_success = upload_to_obs_by_moxing(args.trace, source_url, destination_url)
        except Exception as e:
            log_err(str(e))
